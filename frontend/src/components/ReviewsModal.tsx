        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-slate-700">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-slate-100">{menuItem.name}</h2>
                        <div className="flex items-center gap-2 mt-1">
                            {menuItem.averageRating !== null && menuItem.averageRating > 0 ? (
                                ...
                                <span className="text-sm text-gray-500 dark:text-slate-400">Niciun review încă</span>
                            )}
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                        className="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition text-gray-900 dark:text-slate-100"
                    >
                        <X size={24} />
                    </button>
                </div>
                        ...
                        <div className="mb-6">
                            {showForm || myReview ? (
                                ...
                                <button
                                    ...
                                    className="w-full py-3 bg-gray-900 dark:bg-brand-700 hover:bg-brand-600 dark:hover:bg-brand-600 active:bg-brand-700 dark:active:bg-brand-800 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-gray-200 dark:shadow-none hover:shadow-brand-500/30 dark:hover:shadow-none"
                                >
                                    Adaugă Review
                                </button>
                            )}
                    ...
                    <div className="space-y-4">
                        ...
                        {loading ? (
                            <div className="flex justify-center py-8">
                                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 dark:border-brand-400"></div>
                            </div>
                        ) : reviews.length === 0 ? (
                                ...
                                <div
                                    ...
                                    className={`p-4 rounded-xl border transition-all ${
                                        review.userId === currentUserId
                                            ? 'bg-brand-50 dark:bg-slate-700/80 border-brand-200 dark:border-slate-600 shadow-sm'
